import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { SplitText } from "./customSplitText";

interface ParaElement extends HTMLElement {
  anim?: gsap.core.Animation;
  split?: SplitText;
}

gsap.registerPlugin(ScrollTrigger);

let activeAnims: gsap.core.Animation[] = [];
let activeSplits: SplitText[] = [];

export default function setSplitText() {
  ScrollTrigger.config({ ignoreMobileResize: true });

  // Clean up all active animations and splitText instances from previous run
  activeAnims.forEach((anim) => {
    if (anim.scrollTrigger) {
      anim.scrollTrigger.kill();
    }
    anim.kill();
  });
  activeAnims = [];

  activeSplits.forEach((split) => {
    split.revert();
  });
  activeSplits = [];

  if (window.innerWidth < 900) return;

  const paras: NodeListOf<ParaElement> = document.querySelectorAll(".para");
  const titles: NodeListOf<ParaElement> = document.querySelectorAll(".title");

  const TriggerStart = window.innerWidth <= 1024 ? "top 60%" : "20% 60%";
  const ToggleAction = "play pause resume reverse";

  paras.forEach((para: ParaElement) => {
    para.classList.add("visible");
    
    const split = new SplitText(para, {
      type: "lines,words",
      linesClass: "split-line",
    });
    activeSplits.push(split);

    const anim = gsap.fromTo(
      split.words,
      { autoAlpha: 0, y: 80 },
      {
        autoAlpha: 1,
        scrollTrigger: {
          trigger: para.parentElement?.parentElement,
          toggleActions: ToggleAction,
          start: TriggerStart,
        },
        duration: 1,
        ease: "power3.out",
        y: 0,
        stagger: 0.02,
      }
    );
    activeAnims.push(anim);
  });

  titles.forEach((title: ParaElement) => {
    const existingChars = title.querySelectorAll(".split-char");
    let targetElements: any;

    if (existingChars.length > 0) {
      targetElements = existingChars;
    } else {
      const split = new SplitText(title, {
        type: "chars,lines",
        linesClass: "split-line",
      });
      activeSplits.push(split);
      targetElements = split.chars;
    }

    const anim = gsap.fromTo(
      targetElements,
      { autoAlpha: 0, y: 80, rotate: 10 },
      {
        autoAlpha: 1,
        scrollTrigger: {
          trigger: title.parentElement?.parentElement,
          toggleActions: ToggleAction,
          start: TriggerStart,
        },
        duration: 0.8,
        ease: "power2.inOut",
        y: 0,
        rotate: 0,
        stagger: 0.03,
      }
    );
    activeAnims.push(anim);
  });
}

